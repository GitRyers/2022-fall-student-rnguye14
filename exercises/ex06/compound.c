...
int main(int argc, char *argv[]) {
  ...
  // TODO: Open filename for writing, handle errors
  ...
  float p = 0.0;
  float r = 0.0;
  int parse = fscanf(inFile, "%f%f", &p, &r);  // save return value of parsing attempt below
  ...
  if (parse != EOF) {
    printf("Error: error in parsing the input file.\n");
    return 4;
  }

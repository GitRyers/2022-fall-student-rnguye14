...
void make_cumulative(vector<double> &hist); 
vector<double>::const_iterator naive_find_last_iterator(vector<double>::const_iterator begin, vector<double>::const_iterator end, double v);
vector<double>::const_iterator  fast_find_last_iterator(vector<double>::const_iterator begin, vector<double>::const_iterator end, double v);
...
int main( void )
{
  ...
  for( unsigned int i=0 ; i<sample_num ; i++ ) distribution[ binomial_distribution( generator ) ] += 1./sample_num;
...
vector<double>::const_iterator naive_find_last_iterator(vector<double>::const_iterator begin, vector<double>::const_iterator end, double v) {
  vector<double>::const_iterator it = end - 1;
  while (*it > v && it != begin ) {
    --it; 
  } 
  return it; 
...
vector<double>::const_iterator fast_find_last_iterator(vector<double>::const_iterator begin, vector<double>::const_iterator end, double v) {
  if (end - begin == 1) {
    ...
    vector<double>::const_iterator mid = begin + (end - begin) / 2;
    if (v < *mid) {

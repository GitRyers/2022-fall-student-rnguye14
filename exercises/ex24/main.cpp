...
int main() {
  ...
  while(cin >> word) {
    v.push_back(word); 
    int c = count(v.begin(), v.end(), word); 
    counters[word] = c;
  }
  ...
  for (map<string, int>::const_iterator it = counters.cbegin(); it != counters.cend(); ++it) {
    vector<string> words; 
    int freq = it->second; 
    for (map<string, int>::const_iterator it = counters.cbegin(); it != counters.cend(); ++it) {
      if (it->second == freq) {
  ...
  for (map<int, vector<string>>::const_iterator it = words_by_freq.cbegin(); it != words_by_freq.cend(); it++) {
    cout << endl << "Frequency: " << it->first << endl; 
    for (vector<string>::const_iterator vit = it->second.cbegin(); vit != it->second.cend(); vit++) {
      cout << *vit << endl; 
    }
  }
